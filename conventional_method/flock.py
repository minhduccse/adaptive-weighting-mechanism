...
def main():
    ...
    fail_count = 0
    averdist = 0
    filewriter = open('conventional_method_no_ob.csv', 'a')
    if len(sys.argv)>1:
        filewriter.close()
        filewriter = open('conventional_method.csv', 'a')
        vel_breakdown_filewriter = open('conventional_method_breakdown.csv', 'a')
    b0 = boids[9]
    while running:
        ...
        if len(boids) == 0 or frame_count>900: 
            if frame_count<900:  
